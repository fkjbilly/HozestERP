...
namespace HozestERP.Common.Utils
{
    ...
    public partial class CommonHelper
    {
        ...
        //返回星期中的第几天。
        public static int GetDayOfWeekNum(DateTime Date)
        {
            int iNum = 0;
            ...
            else
            {
                    ...
                    && Convert.ToBoolean(ConfigurationManager.AppSettings["UseSSL"]))
                {
                    ...
                    if (!String.IsNullOrEmpty(sharedSslUrl))
                    {
                         ...
                         * but we cannot reference HozestERP.BusinessLogic.dll assembly.
            ...
            string valueName, bool defaultValue)
        {
            ...
            if (!bool.TryParse(str1, out result))
                throw new CRMException(string.Format("Value must be boolean {0}", valueName));
            return result;
        }
            ...
            string valueName, int defaultValue, bool zeroAllowed, int maxValueAllowed)
        {
            ...
            if (!int.TryParse(str1, out result))
            {
                if (zeroAllowed)
                {
                    throw new CRMException(string.Format("Value must be non negative integer {0}", valueName));
                }
                throw new CRMException(string.Format("Value must be positive integer {0}", valueName));
            }
            ...
            if ((maxValueAllowed > 0) && (result > maxValueAllowed))
                throw new CRMException(string.Format("Value too big {0}", valueName));
            return result;
        }
        ...
        public static string GetDescription(Enum value)
        {
            ...
            return GetDescription(eunmtype, description);
            //DescriptionAttribute[] attributes = (DescriptionAttribute[])value.GetType().GetField(description).GetCustomAttributes(typeof(DescriptionAttribute), false);
        ...
        public static void FillDropDownWithEnumDescription(DropDownList list, Type enumType)
        {
            FillDropDownWithEnumDescription(list, enumType, false);

        }
        public static void FillDropDownWithEnumDescription(DropDownList list, Type enumType,bool isDefault)
        {
            list.Items.Clear();
            string[] strArray = Enum.GetNames(enumType);
            foreach (string item in strArray)
            {
                ...
                string text = GetDescription(enumType, item);
                ListItem ddlItem = new ListItem(text, enumValue.ToString());
                list.Items.Add(ddlItem);
            }

            if (isDefault)
            {
                ListItem ddlItem = new ListItem("---所有---", "-1");
                ddlItem.Selected = true;
                list.Items.Insert(0, ddlItem);
            }
        }

        public static void FillDropDownStatusDescription(DropDownList list, Type enumType, bool isDefault, params string[] strArray)
        {
            list.Items.Clear();
            foreach (string item in strArray)
            {
        ...
        public static string SubString(string word)
        {
            if (word.Length > 10)
            {
                return word.ToString().Substring(0, 10);
            }
            else
            {
                return word;
        ...
        /// <param name="word"></param>
        /// <returns></returns>
        public static string SubStringKeyWord(string word)
        {
            if (word.Length >6)
            {
                return word.ToString().Substring(0, 6)+"...";
            }
            else
            {
                return word;
        ...
        public static void MessageShow(Page page,string msg)
        {
            page.ClientScript.RegisterStartupScript(page.GetType(), "key", "<script>alert('" + msg + "');</script>");
        }

        public static void MessageShow(Page page,string msg,string url)
        {
            page.ClientScript.RegisterStartupScript(page.GetType(), "key", "<script>alert('" + msg + "');window.location.href='" + url + "';</script>");   
        }
        ...
        /// <param name="word"></param>
        /// <returns></returns>
        public static string GetDivision(string word)
        {
            if (string.IsNullOrEmpty(word)||word.Equals("-")||word.Equals("0"))
            {
                return word;
            }
            else
            {
                string date = (decimal.Parse(word) / 100).ToString("0.00");
                return date;
            }
        }
        #endregion

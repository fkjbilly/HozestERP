...
namespace HozestERP.Common.Utils.Html
{
    /// <summary>
    ...
    public partial class ResolveLinksHelper
    {
        ...
        private const string link = "<a href=\"{0}{1}\" target=\"_blank\" rel=\"nofollow\" style=\"text-decoration:underline;cursor:pointer;color:blue;\">{2}</a>";
        private const string link1 = "<a href=\"{0}{1}\" target=\"_blank\" rel=\"nofollow\" style=\"text-decoration:underline;cursor:pointer;color:blue;\" >{2}</a>";
        ...
        /// <returns>Formatted text</returns>
        public static string FormatText(string text, string title)
        {
            if (String.IsNullOrEmpty(text))
                return title;
            ...
            foreach (Match match in matchs)
            {
                if (!match.Value.Contains("://"))
                {
                    text = text.Replace(match.Value, string.Format(info, link1, "http://", match.Value, title));
                }
                else
                {
                    text = text.Replace(match.Value, string.Format(info, link1, string.Empty, match.Value, title));
                }
            }

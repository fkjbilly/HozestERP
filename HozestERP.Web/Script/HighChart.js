...
function GetDateStr(AddDayCount, datetype) {
    var dd = new Date();
...
var HighChart = {
    ChartDataFormate: {
        FormateNOGroupData: function (data) {
         ...
         OriginalFormatGroupData: function (data) {//处理分组数据，数据格式：name：XXX，group：XXX，value：XXX用于折线图、柱形图（分组，堆积）
            var names = new Array();
        ...
        FormatGroupData: function (data, datetype, year, month) {//处理分组数据，数据格式：name：XXX，group：XXX，value：XXX用于折线图、柱形图（分组，堆积）
            var names = new Array();
            var groups = new Array();
            var series = new Array();
            var myDate = new Date();
            if (datetype == "week") {
                for (var i = 6; i >= 0; i--) {
                    names.push(GetDateStr(-i, datetype));
                }
            }
                ...
                var d = dd.getDate();
                for (var i = 1; i <= d; i++) {
                    names.push(pad(month, 2) + "/" + pad(i, 2));
                }
            }
            ...
            for (var i = 0; i < groups.length; i++) {
                ...
                for (var j = 0; j < names.length; j++) {
                    ...
                    for (var k = 0; k < data.length; k++) {
                        if (groups[i] == data[k].group) {
                            if (data[k].name == names[j]) {
                                temp_data.push(data[k].value);
                                exist = true;
                            }
                        }
                    }
                    if (!exist) {
                        temp_data.push(null); //该X轴点无值
                    }
                }
                temp_series = { name: groups[i], data: temp_data };
                series.push(temp_series);
    ...
    ChartOptionTemplates: {
        ...
        Pie_Count: function (data, name, title) {
            var pie_datas = HighChart.ChartDataFormate.FormateNOGroupData(data);
            var option = {
                chart: {
                    plotBackgroundColor: null,
        ...
        Pie_Simple: function (data, name, title) {
            var pie_datas = HighChart.ChartDataFormate.FormateNOGroupData(data);
            var option = {
                chart: {
                    plotBackgroundColor: null,
        ...
        FinanceLine: function (data, name, title) {
            var line_datas = HighChart.ChartDataFormate.OriginalFormatGroupData(data);
            var option = {
                title: {
                    text: title || '',
                    x: -20
        ...
        Line: function (data, name, title, datetype, year, month) {
            var line_datas = HighChart.ChartDataFormate.FormatGroupData(data, datetype, year, month);
            var option = {
                title: {
                    text: title || '',
                    x: -20
        ...
        Bars: function (data, is_stack, is_stack_percent, name, title) {
            var bars_datas = HighChart.ChartDataFormate.OriginalFormatGroupData(data);
            var option = {
                chart: {
                    type: 'column'
                },

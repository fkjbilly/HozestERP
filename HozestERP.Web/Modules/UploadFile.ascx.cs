...
{
    public partial class UploadFile : BaseAdministrationUserControl
    {
        ...
        protected void rpFiles_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName.Equals("DetailDelete"))
            {
                try
                {
                    ...
                    int.TryParse(e.CommandArgument.ToString(), out uploadfileid);

                    base.FileService.DeleteUploadFile(uploadfileid);
                    this.BindRepeater();
                }
                catch (Exception err)
                {

...
{  
    public partial class ProjectXMDeductionSetUp : BaseAdministrationPage, ISearchPage
    {
        /// <summary>
        /// 权限控制
        /// </summary>
        protected override Dictionary<string, string> ButtonSecuritys
        {
            get
            {
                ...
                buttons.Add("imgBtnEdit", "ProjectXMDeductionSetUp.Edit");//编辑
                buttons.Add("imgBtnDelete", "ProjectXMDeductionSetUp.Delete");//删除
                buttons.Add("imgBtnUpdate", "ProjectXMDeductionSetUp.Save");//保存
                buttons.Add("imgBtnCancel", "ProjectXMDeductionSetUp.Cancel");//取消    
                return buttons;
            }
        }
        ...
        public void BindGrid(int paramPageIndex, int paramPageSize)
        {
            ...
            {
                //新增编码行
                this.gvProjectDeduction.EditIndex = XMDeductionSetUpList.Count();
                XMDeductionSetUpList.Add(new XMDeductionSetUp());
            }
            else
            {
                this.gvProjectDeduction.EditIndex = this.RowEditIndex;
            }
            this.Master.BindData(this.gvProjectDeduction, XMDeductionSetUpList, paramPageSize + 1);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void gvProjectDeduction_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            int Id = 0;
            if (int.TryParse(this.gvProjectDeduction.DataKeys[e.RowIndex].Value.ToString(), out Id))
            {
                // 产品信息
        ...
        protected void gvProjectDeduction_RowCancelingEdit(object sender, GridViewCancelEditEventArgs e)
        {
            this.RowEditIndex = -1;
            this.BindGrid(this.Master.PageIndex, this.Master.PageSize);
        }

        protected void gvProjectDeduction_RowEditing(object sender, GridViewEditEventArgs e)
        {
            this.RowEditIndex = e.NewEditIndex;
            this.BindGrid(this.Master.PageIndex, this.Master.PageSize);

            int Id = 0;
            int.TryParse(this.gvProjectDeduction.DataKeys[e.NewEditIndex].Value.ToString(), out Id);
            var row = this.gvProjectDeduction.Rows[e.NewEditIndex];
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void gvProjectDeduction_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {

            int paramparse = 0;
            //decimal param = 0;
            if (int.TryParse(this.gvProjectDeduction.DataKeys[e.RowIndex].Value.ToString(), out paramparse))
            {
                var row = this.gvProjectDeduction.Rows[e.RowIndex];
                ...
                int QID = Convert.ToInt32(this.gvProjectDeduction.DataKeys[e.RowIndex].Value.ToString());
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void gvProjectDeduction_RowDataBound(object sender, GridViewRowEventArgs e)
        {

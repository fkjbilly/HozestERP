...
{
    public partial class OrderInfoProductDetailsSS : BaseAdministrationPage, ISearchPage
    {
        ...
        public void BindGrid(int paramPageIndex, int paramPageSize)
        {
            ...
            //去掉重复的数据源 + 重复数据源
            var OrderInfoSalesDetailsListNew = NotOrderInfoSalesDetailsList.Concat(NotDate).OrderByDescending(p => p.PayDate).ToList();
            ...
            if (BrandType != "-1")
            {
                OrderInfoSalesDetailsListNew = base.XMOrderInfoProductDetailsService.GetOrderInfoSalesDetailsListByBrandType(OrderInfoSalesDetailsListNew, int.Parse(BrandType));
            }

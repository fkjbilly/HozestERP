...
{
    public partial class CWProfitListSSBrandType : BaseAdministrationPage, ISearchPage
    {
        ...
        public void BindGrid(int paramPageIndex, int paramPageSize)
        {
            ...
            if (cbXMProject != "-1")
            {
                if (cbNick == "-1")//项目下的所有店铺
                {
                    ...
                    if (nickList.Count > 0)
                    {
                        nickIdList = nickList.Where(p => p.isEnable == true).Select(p => p.nick_id).ToList();
                    }
                }
                else
                {
            ...
            else
            {
                if (cbNick == "-1")
                {
                    ...
                    nickIdList = NickList.Where(p => p.isEnable == true).Select(a => a.nick_id).ToList();
                }
                else
                {
                    nickIdList.Add(Convert.ToInt32(this.ddlNickList.SelectedValue));
        ...
        private void BindData()
        {
            ...
            //平台类型
            string PlatformTypeId = "-1";
            string BrandType = this.ddlBrandType.SelectedValue;
            ...
            if (CWProfitList.Count > 0)
            {
                ...
                {
                    var list = base.XMOrderInfoService.getXMOrderInfoListByBrandType(CWProfitList, int.Parse(BrandType));
                    CountMoneySum1 = list.Sum(a => a.SalesPrice == null ? 0 : a.SalesPrice.Value);
                }
            }
            ...
            if (xmConsultation.Count > 0)
            {
                ...
                {
                    var list = base.XMApplicationService.GetXMConsultationListByDataByBrandType(xmConsultation, int.Parse(BrandType));
                    CountReturnMoneySum = list.Sum(a => a.SalesPrice == null ? 0 : a.SalesPrice.Value);
                }
            }
            ...
            if (CWJinList.Count > 0)
            {
                ...
                {
                    var list = base.XMOrderInfoProductDetailsService.GetOrderInfoSalesDetailsListByBrandType(CWJinList, int.Parse(BrandType));
                    CountMoneySum4 = list.Sum(a => a.FactoryPrice == null ? 0 : a.FactoryPrice.Value);
                }
            }
            ...
            GetJDProductsPriceList(ref List, SaleList, RejectedList, ReturnList, InventoryList, WareHousesList, int.Parse(BrandType));

            if (BrandType != "-1")
            {
                List = List.Where(x => x.BrandType == BrandType).ToList();
                SaleList = SaleList.Where(x => x.BrandType == BrandType).ToList();
                RejectedList = RejectedList.Where(x => x.BrandType == BrandType).ToList();
                ReturnList = ReturnList.Where(x => x.BrandType == BrandType).ToList();
            }
            ...
            if (xmConsultation.Count > 0)
            {
                ...
                foreach (XMApplication info in xmConsultation)
                {
                    ...
                    if (list != null && list.Count > 0)
                    {
                        ...
                        {
                            if (BrandType != "-1" && Info.BrandType != int.Parse(BrandType))
                            {
                                continue;
                            }
        ...
        public void GetJDProductsPriceList(ref List<JDSelfModel> List, List<XMSaleDeliveryProductDetails> SaleList, List<XMJDSaleRejectedProductDetail> RejectedList
            , List<XMSaleReturnProductDetails> ReturnList, List<XMInventoryInfo> InventoryList, List<int> WareHousesList, int BrandType)
        {
            ...
            {
                if (Info.WareHousesID != -1 && Info.SaleCount > 0)
                {

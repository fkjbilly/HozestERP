...
//文件上传主函数
function UploadFile(paramID, paramInt, paramNums) {
    if (paramNums == "SingleFile") {
        if (!CheckNums(paramID)) {
            alert("只支持单个文件上传！");
            return;
        }
    }
    ...
    var inputFile;
    if (paramInt == 0) {
        inputFile = document.getElementById("file_" + paramID);
    }
    else {
        inputFile = document.getElementById("file_" + initID + "_" + paramID);
    }
    ...
    //将文件信息显示到下方的表格中
    ShowFileInfo(inputFile, paramID);

    //检测添加的文件是否重复,如果重复则删除前面那个节点
    CheckExists(inputFile, paramID);

    //动态创建文件上传控件
    CreateFileControl(paramID, paramNums);
}

function CheckNums(paramID) {
    var divnode = document.getElementById("div_Info_" + paramID)
    if (divnode != null) {
...
//创建显示文件信息的容器DIV
function CreateDiv(paramID) {
    var divTag = document.getElementById("div_Info_" + paramID);
    if ((divTag == null) || (divTag == undefined)) {
        divTag = "<div id='div_Info_" + paramID + "' class='Div_Style1'></div>";
        document.getElementById("table_UploadFile_" + paramID).insertAdjacentHTML("afterEnd", divTag);
    }
}

//显示文件信息
function ShowFileInfo(control, paramID) {
    //创建一个层,表示一行
    var div_row = document.createElement("div");
    //显示文件信息和删除链接
    var tag = "<span class='Attach'>&nbsp;&nbsp;</span>&nbsp;";
    tag += "<span >" + GetFileName(control.value) + "</span>&nbsp;";
    tag += "<a class='Alink' id='A_File_" + initID + paramID + "' onclick=DeleteContainer(this,'" + paramID + "')>删除</a>";
    //将信息插入上面的层中
    div_row.innerHTML = tag;
    //将文件控件也插入该层中
    div_row.appendChild(control);
    //将该层插入div_Info中
    document.getElementById("div_Info_" + paramID).appendChild(div_row);
}
...
//删除文件控件所在的div
function DeleteContainer(control,paramID) {
    document.getElementById("div_Info_" + paramID).removeChild(control.parentElement);
    //如果div_Info所有的子div都被删除了,则删除div_Info
    if (document.getElementById("div_Info_" + paramID).childNodes.length == 0) {
        document.getElementById("div_Info_" + paramID).parentNode.removeChild(document.getElementById("div_Info_" + paramID));
    }
}

//检测添加的文件是否重复,如果重复则删除前面那个节点
function CheckExists(control, paramID) {
    //获取div_Info节点
    var div_Info = document.getElementById("div_Info_" + paramID);
    //将重复的节点删除
        ...
        for (var j = 0; j < div_Info.childNodes[i].childNodes.length; j++) {
            ...
            if (fileControl != control) {
                if (fileControl.value == control.value) {
                    DeleteContainer(fileControl, paramID);
                }
            }
        }
    }
...
//创建文件上传控件
function CreateFileControl(paramID, paramNums) {
    //上传控件初始ID索引加1
    initID++;
    //创建文件上传控件的标记
    var input_File = "<input type='file' name='file_" + initID + "_" + paramID + "' id='file_" + initID + "_" + paramID + "' size='0' class='Input_File' onchange=\"UploadFile('" + paramID + "', " + initID + ", '" + paramNums + "')\" />";
    document.getElementById("file_Container_" + paramID).insertAdjacentHTML("beforeEnd", input_File);
}
...
var Server = {};
Server.RequestMap = {};
Server.MainServletURL = "UILogin.aspx";

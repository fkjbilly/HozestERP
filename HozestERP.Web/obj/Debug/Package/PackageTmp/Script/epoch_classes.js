...
function Epoch(name, mode, targetelement, multiselect) {
   ...
   // wyq
	else if (this.mode = 'flatT' && targetelement) {
	    this.tgt = targetelement;
	    this.calendar.style.position = 'absolute';
	    this.topOffset = this.tgt.offsetHeight; // the vertical distance (in pixels) to display the calendar from the Top of its input element
	    this.leftOffset = 0; 					// the horizontal distance (in pixels) to display the calendar from the Left of its input element
...
{ 
    this.daylist = new Array('日','一','二','三','四','五','六','日','一','二','三','四','五','六'); /*<lang:en>*/
    this.months_sh = new Array('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12');
    this.monthup_title = 'Go to the next month';
    this.monthdn_title = 'Go to the previous month';
    this.clearbtn_caption = 'Clears';
...
Epoch.prototype.createMainHeading = function () //PRIVATE: Creates the primary calendar heading, with months & years
{
	...
	this.yearSelect.owner = this.monthSelect.owner = monthUp.owner = monthDn.owner = this;  //hack to allow us to access this calendar in the events (<fix>??)
...
Epoch.prototype.outputDate = function (vDate, vFormat) //PUBLIC: outputs a date in the appropriate format (DEPRECATED)
{
	...
	return vFormat.replace(/y{1,4}/g, vYear).replace(/mm/g, vMonth).replace(/dd/g, vDay).replace(/hh/g, vHour).replace(/nn/g, vMinute).replace(/ss/g, vSecond).toString("yyyy-MM-dd");
    ...
    if (!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear) {
        if (owner.selectMultiple == true)  //if we can select multiple cells simultaneously, add the currently selected cell's date to the selectedDates array
        {
            if (!cell.selected) //if this cell has been selected
            {
                ...
                {
                    if (owner.tgt.value != null && owner.tgt.value != "") {

                        var lengths = owner.selectedDates.length; //选择的日期总日期数

                        owner.tgt.value = owner.tgt.value + ',' + owner.selectedDates[lengths - 1].dateFormat();
                ...
                for (var i = 0; i < tmp.length; i++) {
                    if (tmp[i].getUeDay() == cell.date.getUeDay()) {
                        tmp.splice(i, 1);
                        // var dateAllString = owner.tgt.value; //取到选中的所有值
                        //var dateString = cell.date.dateFormat(); //取到选中的值
                        if (owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1 > -1) {

                            if (owner.tgt.value.indexOf(cell.date.dateFormat()) == 0) {//取消的时间是第一个字符串
                                var dateSubDu1 = owner.tgt.value.substr(
                                 owner.tgt.value.substr(0, owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1
                                                        + owner.tgt.value.substr(owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1,  
                                                        owner.tgt.value.indexOf(cell.date.dateFormat())).length).length,  owner.tgt.value.length - 1);
                                owner.tgt.value = dateSubDu1;

                            } else {
                                ////取消的日期是最后一个字符串
                                //owner.tgt.value.substr(owner.tgt.value.substr(0, owner.tgt.value.indexOf(cell.date.dateFormat()) + owner.tgt.value.substr(owner.tgt.value.indexOf(cell.date.dateFormat()), owner.tgt.value.indexOf(cell.date.dateFormat())).length).length, owner.tgt.value.length - 1)
                                if (owner.tgt.value.substr(owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1, owner.tgt.value.length - 1)== "") 
                                {
                                    var dateSubUp1 = owner.tgt.value.substr(0, owner.tgt.value.indexOf(cell.date.dateFormat()));
                                    owner.tgt.value = dateSubUp1;
                                }
                                ////取消的日期是中间一个字符串
                                else {
                                    var dateSubUp = owner.tgt.value.substr(0, owner.tgt.value.indexOf(cell.date.dateFormat())); //截取选择值前面的日期字符串
                                    var dateSubDu = owner.tgt.value.substr(owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1, owner.tgt.value.length - 1); 
                                    //owner.tgt.value.substr(owner.tgt.value.substr(0, owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1 + owner.tgt.value.substr(owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1, owner.tgt.value.indexOf(cell.date.dateFormat()) + cell.date.dateFormat().toString().length + 1).length).length, owner.tgt.value.length - 1);
                                    owner.tgt.value = dateSubUp + dateSubDu;
        ...
        else //if we can only select one cell at a time
        {
            ...
            if (owner.tgt) //if there is a target element to place the value in, do so
            {
                owner.tgt.value = owner.tgt.value + ',' + owner.selectedDates[0].dateFormat();
                if (owner.mode == 'popup') {
                    owner.hide();
                }
            }

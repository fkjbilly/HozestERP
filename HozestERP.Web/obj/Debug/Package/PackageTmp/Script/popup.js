...
Popup.prototype = {
    ...
    setBackgroundSize: function () {
        ...
        //G('dialogBoxBG').style.height = "100%";
    },
    lastBuild: function() {
        ...
        if (this.config.contentType == 1) {
            ...
            Event.observe(G('dialogBoxClose'), "click", this.forCallback.bindAsEventListener(this), false);
...
var popp;
function ShowIframe(url, w, h, t, target) //显示有滚动条的iframe
{
    var pop = new Popup({
        contentType: 1,
        scrollType: 'yes',
    ...
    pop.setContent("callBack", ShowIframeCallBackServer); //回调函数
...
function ShowIframeNoScroll(url, w, h, t, target) //显示没有滚动条的iframe
{
    var pop = new Popup({
        contentType: 1,
        scrollType: 'no',
...
function ShowIframeCallBackServer(para) {
    var target = para["str"];
    if (target != null) {
        target();
    }

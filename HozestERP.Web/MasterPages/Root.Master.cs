...
{
    public partial class Root : BaseAdministrationMasterPage
    {
        ...
        public int PageSize
        {
            get {
                try
                {
                    if (Convert.ToInt32( this.GridNevigator1.PageSize) <= 0)
                    {
                        return 10;
                    }
        ...
        public void SetGridHeadStyle(GridView myGrid)
        {
            ...
            {
                if (myGrid.HeaderRow.Cells[j].RowSpan > 1)
                {
                    myGrid.HeaderRow.Cells[j].Attributes.Add("class", "GridHeard2");
                }
                else
                {
                ...
                //如果有设置排序码的话就设置排序事件
                if (CellsCount == myGrid.Columns.Count && myGrid.Columns[j].SortExpression != null & myGrid.Columns[j].SortExpression != string.Empty)
                {
                    ...
                    {
                        if (this.GridViewSortDir == GridViewSortDirEnum.DESC)
                        {
        ...
        protected void btnGridViewSort_Click(object sender, EventArgs e)
        {
            if (this.hidGridViewSortField.Value == this.GridViewSortField)
            {
                if (this.GridViewSortDir == GridViewSortDirEnum.DESC)
                {
                    this.GridViewSortDir = GridViewSortDirEnum.ASC;
                }
                else
                {
                    this.GridViewSortDir = GridViewSortDirEnum.DESC;
                }
            }
            else
            {
        ...
        /// GridView 排序规则
        /// </summary>
        public GridViewSortDirEnum GridViewSortDir
        {
            get
            {
                try
                {
                    return (GridViewSortDirEnum)ViewState["GridViewSortDirEnum"];
                }
                catch
                {
                }
                return GridViewSortDirEnum.ASC;
            }
            set
            {
    ...
    public enum GridViewSortDirEnum
    {
        DESC,
        ASC
    }

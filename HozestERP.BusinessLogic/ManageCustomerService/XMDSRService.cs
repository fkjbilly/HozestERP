...
namespace HozestERP.BusinessLogic.ManageCustomerService
{
    public partial class XMDSRService: IXMDSRService
    {
        ...
        /// get to XMDSR list
        /// </summary>
        public List<XMDSR> GetXMDSRList(int ddlNickId, int ProjectTypeId, string Month)
        {		
            var query = from p in this._context.XMDSRs
                        where (p.NickId == ddlNickId || ddlNickId == -1) && (p.PlatfromTypeId == ProjectTypeId || ProjectTypeId == -1) && p.IsEnabled == false && (p.Month == Month || Month=="")

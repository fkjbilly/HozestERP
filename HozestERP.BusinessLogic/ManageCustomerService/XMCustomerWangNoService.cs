...
namespace HozestERP.BusinessLogic.ManageCustomerService
{
    public partial class XMCustomerWangNoService: IXMCustomerWangNoService
    {
        ...
        public XMWangNo GetXMCustomerWangNoName(int WangNoID)
        {
            var query = from p in this._context.XMWangNoes
                        where p.ID.Equals(WangNoID)
                        select p;
            return (XMWangNo)(query.ToList())[0];
        }
        ...
        public List<XMCustomerWangNo> GetXMCustomerWangNoByCustomerID(int CustomerID)
        {
            var query = from p in this._context.XMCustomerWangNoes
                        where p.CustomerID == CustomerID && p.IsEnabled == false
                        select p;
            return query.ToList();
        }

        public List<XMCustomerWangNo> GetXMCustomerWangNoByWangNoID(int WangNoID)
        {
            var query = from p in this._context.XMCustomerWangNoes
                        where p.WangNoID == WangNoID && p.IsEnabled == false
                        select p;
            return query.ToList();
        }

...
namespace HozestERP.BusinessLogic.Enterprises
{
    public partial class ProvinceWarehouseSpareService : IProvinceWarehouseSpareService
    {
        ...
        public List<ProvinceWarehouseSpare> GetProvinceWarehouseSpareByParm(int ProvinceWarehouseID, int Sequence, int SpareWarehouse)
        {
            var query = from p in this._context.ProvinceWarehouseSpares
                        where p.IsEnabled == false
                        && (Sequence == -1 || p.Sequence == Sequence)
                        && (SpareWarehouse == -1 || p.SpareWarehouseID == SpareWarehouse)
                        && p.ProvinceWarehouseID == ProvinceWarehouseID
                        select p;
            return query.ToList();
        }

        public List<ProvinceWarehouseSpare> GetProvinceWarehouseSpareByProvinceWarehouseID(int ProvinceWarehouseID)
        {
            var query = from p in this._context.ProvinceWarehouseSpares
                        where p.ProvinceWarehouseID == ProvinceWarehouseID
                        && p.IsEnabled == false

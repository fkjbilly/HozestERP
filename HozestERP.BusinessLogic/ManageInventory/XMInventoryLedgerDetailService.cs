...
namespace HozestERP.BusinessLogic.ManageInventory
{
    public partial class XMInventoryLedgerDetailService : IXMInventoryLedgerDetailService
    {
        ...
        /// <returns></returns>
        public List<XMInventoryLedgerDetail> GetXMInventoryLedgerDetailListByParm(string begin, string end, int wareHouseId, string platformMerchantCode)
        {
            ...
            var query = from p in this._context.XMInventoryLedgerDetails
                        where (wareHouseId == -1 || p.WarehouseId == wareHouseId)
                      && ((begin == "" && end == "") || (p.BizDate >= pdBeginTime && p.BizDate < pdEndTime))
                      && (platformMerchantCode == "" || p.PlatformMerchantCode.Contains(platformMerchantCode))
                        orderby p.BizDate descending
                        select p;
            return query.ToList();
        }
        /// <summary>
        ...
        /// <param name="platformMerchantCode"></param>
        /// <returns></returns>
        public List<XMInventoryLedgerDetail> GetXMInventoryLedgerDetailListByParm(int wareHouseId, string platformMerchantCode)
        {
            var query = from p in this._context.XMInventoryLedgerDetails
                        where (wareHouseId == -1 || p.WarehouseId == wareHouseId)
                         && (platformMerchantCode == "" || p.PlatformMerchantCode.Contains(platformMerchantCode))
        ...
        //更具业务状态查询所有记录
        public List<XMInventoryLedgerDetail> GetXMInventoryLedgerDetailListByParm(int wareHouseId, string platformMerchantCode, int status)
        {
            var query = from p in this._context.XMInventoryLedgerDetails
                        where (wareHouseId == -1 || p.WarehouseId == wareHouseId)
                          && (platformMerchantCode == "" || p.PlatformMerchantCode.Contains(platformMerchantCode))

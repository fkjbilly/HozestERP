...
namespace HozestERP.BusinessLogic.ManageInventory
{
    public partial class XMSaleReturnService : IXMSaleReturnService
    {
        ...
        public List<XMSaleReturn> GetXMSaleReturnListByDate(DateTime Begin, DateTime End, List<int?> WareHousesList)
        {
            var query = from p in this._context.XMSaleReturns
                        where p.IsEnable == false
        ...
        /// <returns></returns>
        public List<XMSaleReturn> GetXMSaleReturnListByParm(string saleReturnCode, string begin, string end, string wareHoursesIDs, int status,int projectId)
        {
            int?[] wareHoursesID = Array.ConvertAll<string, int?>(wareHoursesIDs.Split(','), delegate(string s) { return int.Parse(s); });
            ...
            var query = from p in this._context.XMSaleReturns
                        ...
                        where p.IsEnable == false
                        && (saleReturnCode == "" || p.Ref.Contains(saleReturnCode))
                        && ((begin == "" && end == "") || (p.BizDt >= purBeginTime && p.BizDt < purEndTime))
                        && wareHoursesID.Contains(p.WarehouseId)
        ...
        /// <returns></returns>
        public List<XMSaleReturn> GetXMSaleReturnListByPorjectID(string saleReturnCode, string begin, string end, int wareHoursesID, int status, string projectids, int projectId)
        {
            ...
            var query = from p in this._context.XMSaleReturns
                        ...
                        && (saleReturnCode == "" || p.Ref.Contains(saleReturnCode))
                        && ((begin == "" && end == "") || (p.BizDt >= purBeginTime && p.BizDt < purEndTime))
                        && (wareHoursesID == -1 || p.WarehouseId == wareHoursesID)
        ...
        /// <summary>
        /// get to XMSaleReturn  List by wareHousesID
        /// </summary>
        /// <param name="wareHousesID"></param>
        /// <returns></returns>
        public List<XMSaleReturn> GetXMSaleReturnListByWareHoursesID(int wareHousesID)
        {
            var query = from p in this._context.XMSaleReturns
                        where p.IsEnable == false

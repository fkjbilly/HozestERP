...
namespace HozestERP.BusinessLogic.ManageInventory
{
    public partial class XMActivityTypeService : IXMActivityTypeService
    {
        ...
        {
            var query = from p in this._context.XMActivityTypes
                        where p.Id.Equals(id) && p.IsEnale == false
                        select p;
        ...
        /// <returns></returns>
        public XMActivityType GetXMActivityTypeByActivityTypeName(string activityTypeName)
        {
            var query = from p in this._context.XMActivityTypes
                        where p.IsEnale == false
                        && p.ActivityName == activityTypeName
                        select p;
            return query.SingleOrDefault();
        }
        /// <summary>
        ...
        /// <param name="activityTypeName"></param>
        /// <returns></returns>
        public List<XMActivityType> GetXMActivityTypeInfoByParm(string activityTypeName)
        {
            var query = from p in this._context.XMActivityTypes
                        where p.IsEnale == false
                        && (activityTypeName == "" || p.ActivityName.Contains(activityTypeName))
                        orderby p.CreateDate descending
                        select p;
            return query.ToList();
        }

...
namespace HozestERP.BusinessLogic.ManageProject
{
    public partial class XMOrderInfoPlanRecordService: IXMOrderInfoPlanRecordService
    {
        ...
        public IList<XMOrderInfoPlanRecord> GetByStatus(string status)
        {
            if (string.IsNullOrEmpty(status)) return null;
            var query = from m in _context.XMOrderInfoPlanRecords
                        where m.Status == status
                        select m;
            return query.ToList<XMOrderInfoPlanRecord>();
        }
